		...
		for(i=0;i<(MAX_NETIF_HW_TABLE_SIZE<<1);i++){
			if(rg_db.systemGlobal.intfArpRequest[i].finished==0){
				...
				if(pPktHdr->ipv4Sip==rg_db.systemGlobal.intfArpRequest[i].reqIp && ret==SUCCESS){
					...
					if(_rtk_rg_checkARPForL2TP_PPTP(&rg_db.systemGlobal.interfaceInfo[i-MAX_NETIF_HW_TABLE_SIZE].storedInfo.wan_intf,i,pPktHdr)==RG_RET_SUCCESS){
						break;
					}
				}
			}
...
{
	int neighborIdx=-1,macIdx=-1,l3Idx,i,netIfIdx=-1;
	int intfND_idx=-1,staticND_idx=-1;
	...
	if((rg_db.pktHdr->tagif&ICMPV6_TAGIF) && ((pPktHdr->ICMPv6Type==0x88)||(pPktHdr->ICMPv6Type==0x87)))
	{
		...
		if(intfND_idx>=0 || staticND_idx>=0){
			...
			}else if((intfND_idx>=MAX_NETIF_SW_TABLE_SIZE)&&(rg_db.systemGlobal.interfaceInfo[netIfIdx].storedInfo.wan_intf.wan_intf_conf.wan_type==RTK_RG_DSLITE)){
			...
			if(intfND_idx>=0){
				if(rg_db.systemGlobal.intfNeighborDiscovery[intfND_idx].ipv6GwMacReqCallBack!=NULL)
				{
					//DEBUG("wan intf is %d, function pointer is %p",i,rg_db.systemGlobal.intfNeighborDiscovery[intfND_idx].ipv6GwMacReqCallBack);
					//DEBUG("rg_db.v6neighbor[%d].rtk_v6neighbor.l2Idx is %d",neighborIdx,macIdx);
					rg_db.systemGlobal.intfNeighborDiscovery[intfND_idx].ipv6GwMacReqCallBack(rg_db.systemGlobal.intfNeighborDiscovery[intfND_idx].reqIp.ipv6_addr,macIdx);
					//DEBUG("");
				}
				rg_db.systemGlobal.intfNeighborDiscovery[intfND_idx].finished=1;
			}
			if(staticND_idx>=0){
				if(rg_db.systemGlobal.staticRouteNBDiscovery[staticND_idx].ipv6GwMacReqCallBack!=NULL){
					rg_db.systemGlobal.staticRouteNBDiscovery[staticND_idx].ipv6GwMacReqCallBack(rg_db.systemGlobal.staticRouteNBDiscovery[staticND_idx].reqIp.ipv6_addr,macIdx);
				}
				rg_db.systemGlobal.staticRouteNBDiscovery[staticND_idx].finished=1;
			}
		}
	}
	return RG_RET_SUCCESS;
...
void _rtk_rg_naptShortcutUpdate(rtk_rg_pktHdr_t *pPktHdr,int dir,int naptIdx, int isNapt, int isBridge)
{
	...
	shortcutIdx=_rtk_rg_shortcutHashIndex(pPktHdr->ipv4Sip,pPktHdr->sport,pPktHdr->dport);
	...
	{
		shortcutIdx=_rtk_rg_shortcutHashIndex(pPktHdr->ipv4Dip,pPktHdr->dport,rg_db.naptOut[naptIdx].extPort);
...
void _rtk_rg_removeTunnelTag(struct sk_buff *skb, rtk_rg_pktHdr_t *pPktHdr)
{
	...
	if(pPktHdr->tagif&IPV4_TAGIF)*(unsigned short *)(skb->data+(VLAN_ETH_ALEN<<1)+vlanOffset)=htons(RG_IPV4_ETHERTYPE);
	else *(unsigned short *)(skb->data+(VLAN_ETH_ALEN<<1)+vlanOffset)=htons(RG_IPV6_ETHERTYPE);
	...
	if (tmpptr!=NULL){
		...
		if(rg_db.redirectHttpRsp.statusCode==simple_strtol(statusCodeptr, NULL, 0)){
			...
			if(direct!=NAPT_DIRECTION_ROUTING){
				...
				//Upstream
				i=_rtk_rg_shortcutHashIndex(pPktHdr->ipv4Dip,pPktHdr->dport,pPktHdr->sport);
				...
				//Downstream
				i=_rtk_rg_shortcutHashIndex(pPktHdr->ipv4Sip,pPktHdr->sport,pPktHdr->dport);
				for(j=i;j<i+MAX_NAPT_SHORTCUT_WAYS;j++){
					pNaptSc=&rg_db.naptShortCut[j];
					if(pNaptSc->direction==NAPT_DIRECTION_ROUTING){
		...
		if((pPktHdr->tagif&TCP_TAGIF) || (pPktHdr->tagif&UDP_TAGIF)||(pPktHdr->tagif&ICMP_TAGIF))
		{
			...
			if(	(rg_db.systemGlobal.pValidUsNaptPriorityRuleStart!= NULL) || (rg_db.systemGlobal.pValidDsNaptPriorityRuleStart!=NULL)){//check naptFilter while any rule is set. else save the time.
				...
				if(naptFilterRet==RG_FWDENGINE_RET_DROP){
					return RG_FWDENGINE_RET_DROP;
				}else if (naptFilterRet==RG_FWDENGINE_RET_ACL_TO_PS){
...
__IRAM_FWDENG
uint32 _rtk_rg_shortcutHashIndex(uint32 srcIP, uint16 srcPort, uint16 destPort)
{
	//one-way
	...
	*(u32*)(skb->data+skb->len)=0; //save null point into end of skb data.(for trace filter debug)
	...
	if(pPktHdr->tagif&PPPOE_TAGIF){
		...
		*(uint16 *)(bufptr + 24 + vlan_offset) = htons(inet_chksum(bufptr + 14 + vlan_offset,20));
		//TCP checksum
		*(uint16 *)(bufptr + 50 + vlan_offset) = htonl(inet_chksum_pseudo(bufptr + 34 + vlan_offset,totalLen - 20,*(uint32 *)(bufptr + 26 + vlan_offset),*(uint32 *)(bufptr + 30 + vlan_offset),0x6));
	}
#endif
...
{
	TRACE("Filter VLAN(%d) port(%d) extPort(%d)",vlanId, port, extPort);
...
ADD_SVL_LUT:
	...
	do
	{
			...
			(memcmp(rg_db.lut[search_index].rtk_lut.entry.l2UcEntry.mac.octet,pPktHdr->pSmac,ETHER_ADDR_LEN)==0))
		{
			...
			{
				if(pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_L2_LEARN_LIMIT_PERPORT /* SA learning limit */ || pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_UNKNOWN_SA /* unknown SA action */)
				{
...
//RET_TO_PS:
				...
				else
				{
					...
					{
#ifdef __KERNEL__
						rtk_rg_wq_union_t *pppoeLCPWq=wq_callbackAssign();
						struct sk_buff *lcp_skb=NULL;
						INIT_WORK(&(pppoeLCPWq->work), wq_do_pppoeLCPStateCallBack);
						lcp_skb=rtk_rg_skbCopyToPreAllocSkb(skb);
						if(lcp_skb){
							pppoeLCPWq->data=(void *)lcp_skb;
							queue_work(rg_kernel.rg_callbackWQ,&(pppoeLCPWq->work));
						}
#else
						rg_db.systemGlobal.initParam.pppoeLCPStateCallBack((void *)skb);
#endif
					}
#endif
					if((rg_db.algFunctionMask & RTK_RG_ALG_PPPOE_PASSTHROUGH_BIT) > 0)		/* Handle broadcast packet to routing WAN if PPPoE pass through is turn on*/
			...
			if(pPktHdr->ingressPort==RTK_RG_EXT_PORT0) //from EXT0, wireless WAN
			{
				...
				{				
					...
					else
					{
						...
						if(pRxDesc->opts3.bit.reason==RG_CPU_REASON_NORMAL_FWD)
						{
...
#if 0
				if(pRxDesc->opts3.bit.reason==RG_CPU_REASON_UKNOWNDA_IPV6MC) //Unkown IPv6 MC
				{
...
#if !defined(CONFIG_APOLLOFE_DRIVER_TESTCHIP)
		//20160323LUKE: for the host-policing mib check!
		pPktHdr->smacL2Idx=pIPv6ConnList->smacL2Idx;
#endif
#endif
...
#if defined (CONFIG_XDSL_NEW_HWNAT_DRIVER)
rtk_rg_fwdEngineReturn_t _rtk_rg_dateFlow_multicast_process(rtk_rg_pktHdr_t *pPktHdr)
{
	int32 mcRet;
...
rtk_rg_fwdEngineReturn_t _rtk_rg_fwdEngineSlowPath(struct sk_buff *skb,rtk_rg_pktHdr_t *pPktHdr)
{
	...
	for(ret=0;ret<rg_db.systemGlobal.wanIntfTotalNum;ret++){
		if(rg_db.systemGlobal.hwAclIdx_for_trap_dhcp[rg_db.systemGlobal.wanIntfGroup[ret].index]!=FAIL){
			if(pPktHdr->pRxDesc->opts3.bit.reason==rg_db.systemGlobal.hwAclIdx_for_trap_dhcp[rg_db.systemGlobal.wanIntfGroup[ret].index]+RG_CPU_REASON_ACL_TRAP_START/*ACL reason base*/){
...
/*  for 8685 sync lut table from hardware to software	*/
	...
	{
		//XDSL:multicast data
		ret=_rtk_rg_dateFlow_multicast_process(pPktHdr);
		if(ret!=RG_FWDENGINE_RET_CONTINUE)
			return ret;		
	}
#endif
...
#if defined(CONFIG_XDSL_NEW_HWNAT_DRIVER)
		if(pPktHdr->fwdDecision!=RG_FWD_DECISION_PPPOE_MC  && pPktHdr->fwdDecision!=RG_FWD_DECISION_FLOW_MC && pPktHdr->fwdDecision!=RG_FWD_DECISION_NO_PS_BC)
#else
		if(pPktHdr->fwdDecision!=RG_FWD_DECISION_PPPOE_MC && pPktHdr->fwdDecision!=RG_FWD_DECISION_NO_PS_BC)
#endif
		{	
...
rtk_rg_fwdEngineReturn_t _fwdEngineEntrypoint(struct re_private *cp, struct sk_buff *skb, void *pRxDescPtr)
{
	...
	if(!rg_db.systemGlobal.ipv4_shortcut_off){
		...
		if((pPktHdr->tagif&IPV4_TAGIF)&&(!((pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_L2_LEARN_LIMIT_PERPORT)||(pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_NAT_OVER_MTU)))){
#if 1
...
#if !defined(CONFIG_APOLLOFE_DRIVER_TESTCHIP)
				//20160323LUKE: for the host-policing mib check!
				pPktHdr->smacL2Idx=pNaptSc->smacL2Idx;
#endif
#endif
...
OUT_OF_SHORTCUT:
	...
	if(!rg_db.systemGlobal.ipv6_shortcut_off){
		...
		/* 192: SA learning limit */ 
		/* 022: Over MTU Size must calculate checksum by software.*/
		if((pPktHdr->tagif&IPV6_TAGIF)&&(!((pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_L2_LEARN_LIMIT_PERPORT)||(pPktHdr->pRxDesc->opts3.bit.reason==RG_CPU_REASON_NAT_OVER_MTU)))){
...
#if !defined(CONFIG_APOLLOFE_DRIVER_TESTCHIP)
				//20160323LUKE: for the host-policing mib check!
				pPktHdr->smacL2Idx=pNaptV6Sc->smacL2Idx;
#endif
#endif
...
#if defined(CONFIG_RTL9602C_SERIES) && !defined(CONFIG_APOLLOFE_DRIVER_TESTCHIP)
	...
	switch(ret){
		...
		default:			
			...
			if(rg_db.lut[pPktHdr->smacL2Idx].host_idx_valid){
				if(pPktHdr->pRxDesc->opts1.bit.data_length<=RG_MIN_MRU)
					rg_db.hostPoliceList[rg_db.lut[pPktHdr->smacL2Idx].host_idx].count.rx_count+=RG_MIN_MRU+4;	//add crc
				else
					rg_db.hostPoliceList[rg_db.lut[pPktHdr->smacL2Idx].host_idx].count.rx_count+=(pPktHdr->pRxDesc->opts1.bit.data_length+4);	//add crc
...
{
	rtk_rg_fwdEngineReturn_t ret=_fwdEngineEntrypoint(cp, skb, pRxDescPtr);
#if defined(CONFIG_RTL9602C_SERIES) && !defined(CONFIG_APOLLOFE_DRIVER_TESTCHIP)
...
#ifdef CONFIG_DUALBAND_CONCURRENT
	if(*((unsigned short *)(&skb->data[ETHER_ADDR_LEN<<1]))==SLAVE_SSID_TAG_ETH)
		pRxInfo->opts3.bit.dst_port_mask=0x10;
#endif
