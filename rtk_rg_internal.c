...
rtk_rg_wq_union_t *wq_callbackAssign(void)
{
	...
	rg_kernel.rg_cbUnionCurrentIdx%=MAX_CALLBACK_UNION_NUM;
	return ret;
}
...
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.interfaceAddByHwCallBack(&p_callback_work->interfaceCallBack.intfInfo,&p_callback_work->interfaceCallBack.intfIdx);
}
void wq_do_interfaceDelByHwCallBack(struct work_struct *p_work)
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.interfaceDelByHwCallBack(&p_callback_work->interfaceCallBack.intfInfo,&p_callback_work->interfaceCallBack.intfIdx);
}
...
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.pppoeBeforeDiagByHwCallBack(&p_callback_work->pppoeBeforeDialCallBack.pppoeBeforeInfo,&p_callback_work->pppoeBeforeDialCallBack.wanIdx);
}
...
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.pptpBeforeDialByHwCallBack(&p_callback_work->pptpBeforeDialCallBack.pptpBeforeInfo,&p_callback_work->pptpBeforeDialCallBack.wanIdx);
}
void wq_do_l2tpBeforeDialByHwCallBack(struct work_struct *p_work)
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.l2tpBeforeDialByHwCallBack(&p_callback_work->l2tpBeforeDialCallBack.l2tpBeforeInfo,&p_callback_work->l2tpBeforeDialCallBack.wanIdx);
}
void wq_do_pppoeDsliteBeforeDialByHwCallBack(struct work_struct *p_work)
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.pppoeDsliteBeforeDialByHwCallBack(&p_callback_work->pppoeDsliteBeforeDialCallBack.pppoeDsliteBeforeInfo,&p_callback_work->pppoeDsliteBeforeDialCallBack.wanIdx);
}
...
void wq_do_pppoeLCPStateCallBack(struct work_struct *p_work)
{
	rtk_rg_wq_union_t *p_callback_work = container_of(p_work, rtk_rg_wq_union_t, work);
	rg_db.systemGlobal.initParam.pppoeLCPStateCallBack(&p_callback_work->data);
}
#endif
