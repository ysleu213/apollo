...
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
	rtk_rg_callback_pipe_cmd("`for FILE in $(ls /sys/class/net/ | grep 'eth0.2.'); do vconfig rem $FILE > /dev/null 2>&1; done`");
	rtk_rg_callback_pipe_cmd("`for FILE in $(ls /sys/class/net/ | grep 'eth0.3.'); do vconfig rem $FILE > /dev/null 2>&1; done`");
	rtk_rg_callback_pipe_cmd("`for FILE in $(ls /sys/class/net/ | grep 'eth0.4.'); do vconfig rem $FILE > /dev/null 2>&1; done`");
	rtk_rg_callback_pipe_cmd("`for FILE in $(ls /sys/class/net/ | grep 'eth0.5.'); do vconfig rem $FILE > /dev/null 2>&1; done`");
//#else
#endif
...
//#endif
		...
		if(intfInfo->lan_intf.port_mask.portmask&(0x1<<RTK_RG_EXT_PORT0)
#ifdef CONFIG_DUALBAND_CONCURRENT
			||(rg_db.systemGlobal.enableSlaveSSIDBind && intfInfo->lan_intf.port_mask.portmask&(0x1<<RTK_RG_EXT_PORT1))
#endif
...
int _rtk_rg_bindingAddByHwCallBack(rtk_rg_bindingEntry_t* bindingInfo){	
	...
	{	
		...
		if(rg_db.systemGlobal.interfaceInfo[bindingInfo->wan_intf_idx].storedInfo.is_wan && rg_db.systemGlobal.interfaceInfo[bindingInfo->wan_intf_idx].storedInfo.wan_intf.wirelessWan==RG_WWAN_WIRED){
			rtk_rg_intfInfo_t* intfInfo=&rg_db.systemGlobal.interfaceInfo[bindingInfo->wan_intf_idx].storedInfo;
